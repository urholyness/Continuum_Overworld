...
export const callOpenAI = async ({ system, user }) => {
    const apiKey = process.env.AEGIS_OPENAI_API_KEY;
    if (!apiKey)
        throw new Error("AEGIS_OPENAI_API_KEY missing - required for real extraction");
...
export const callClaude = async ({ system, user }) => {
    ...
    if (!apiKey)
        throw new Error("AEGIS_CLAUDE_API_KEY missing - required for real extraction");
...
// Gemini adapter  
export const callGemini = async ({ system, user }) => {
    const apiKey = process.env.AEGIS_GEMINI_API_KEY;
    if (!apiKey)
        throw new Error("AEGIS_GEMINI_API_KEY missing - required for real extraction");
...
export const callConsensus = async ({ system, user }) => {
    const providers = [];
    // Add available providers based on env vars
    if (process.env.AEGIS_OPENAI_API_KEY)
        providers.push({ name: "openai", fn: callOpenAI });
    if (process.env.AEGIS_CLAUDE_API_KEY)
        providers.push({ name: "claude", fn: callClaude });
    if (process.env.AEGIS_GEMINI_API_KEY)
    ...
    for (const result of successful) {
        if (result.data.kpis)
            allKpis.push(...result.data.kpis);
        if (result.data.contexts)
            allContexts.push(...result.data.contexts);
    }
    // Basic deduplication by kpi_id and value
    const uniqueKpis = Array.from(new Map(allKpis.map(k => [`${k.kpi_id}-${k.value}-${k.unit}`, k])).values());
