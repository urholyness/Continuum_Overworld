...
export async function parsePdfToJsonl(company, year) {
    ...
    const lines = pages.map((text, i) => JSON.stringify({ page: i + 1, text }));
    await fs.writeFile(outPath, lines.join("\n"));
    return { outPath, pages: pages.length };
}
export async function runParse(companies, years) {
...
if (process.argv[1]?.endsWith("pdf-parse.ts")) {
    ...
    if (!companies.length)
        throw new Error("Provide at least one company.");
