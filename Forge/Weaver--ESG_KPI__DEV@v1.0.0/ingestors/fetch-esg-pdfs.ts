...
type Providers = { mode: "real"; esg_docs: { companies: string[]; years: number[] } };

const companySeeds: Record<string, string[]> = {
...
function yearRegex(y: number) {
  return new RegExp(`(?:^|[^\\d])${y}(?:[^\\d]|$)`);
}
function isPdfLink(href: string | undefined) {
...
export async function loadProviders(): Promise<Providers> {
  ...
  const parsed = YAML.parse(cfg) as Providers;
  if (parsed.mode !== "real") throw new Error("Weaver is not in REAL mode.");
  return parsed;
}

async function findPdfUrl(company: string, year: number): Promise<string> {
  ...
  for (const url of seeds) {
    try {
      ...
      const links = $("a[href]")
        .map((_: any, a: any) => ($(a).attr("href") || "").trim())
        .get()
        .filter(isPdfLink)
        .filter((h: string) => yearRegex(year).test(h));
      if (links.length) {
...
export async function fetchEsgPdf(company: string, year: number) {
